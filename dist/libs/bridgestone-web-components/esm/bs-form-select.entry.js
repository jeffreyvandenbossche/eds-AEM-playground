import { r as registerInstance, h, H as Host, g as getElement } from './index-17dff65c.js';
...
const BsFormSelect = class {
  constructor(hostRef) {
    ...
    this.focus = () => {
      ...
      for (let i = 0; i < this.selectElRef.childNodes.length; i += 1) {
        if (this.selectElRef.childNodes[i].selected) {
          this.activeOption = i;
        }
      }
    };
  ...
  handleSelect(e) {
    ...
    if (nativeSelect.options[selectedIndex].disabled)
      return;
  ...
  render() {
    ...
    return (h(Host, { focused: this.focused }, h("bs-form-field", { ref: (el) => {
        ...
        this.selectWrap = el;
      }, id: this.name, class: "custom-select", onFocus: !this.focused ? this.focus : this.focusOut, onClick: !this.focused && this.focus, tabIndex: 0 }, h("span", { class: "custom-select-value" }, (_b = this.value) === null || _b === void 0 ? void 0 : _b.text), h("select", { ref: (el) => {
        this.selectElRef = el;
        return this.selectElRef;
      }, class: "hidden", required: this.required, id: `hidden-${this.name}`, name: this.name, tabIndex: -1 }, this.parsedOptions.map((option, index) => (h("option", { selected: option.selected, "aria-selected": option.selected, value: option.value, disabled: option.disabled, "data-index": index }, option.text)))), h("div", { "aria-labelledby": this.name }, h("ul", { class: "custom-options-wrapper", "aria-haspopup": "listbox", "aria-expanded": "false", role: "group" }, this.parsedOptions.map((option, index) => (h("li", { ref: (el) => {
        this.customOptions.push(el);
      }, role: "option", class: `custom-option ${option.disabled ? 'is-disabled' : ''}`, tabindex: -1, "aria-disabled": option.disabled ? 'true' : false, "aria-selected": option.selected, "data-index": index }, option.text)))))), h("label", { slot: "form-label", class: "form-label", htmlFor: this.name, title: this.label }, this.label, this.required ? h("span", { "aria-hidden": "true" }, "\u00A0*") : ''))));
  }
